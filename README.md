...
Proxy pool selection rotates per probe. max_attempts defaults to 1 (range 1–20); failed or rejected attempts advance through the pool, sharing timeout_seconds (default 15, range 1–180). retry_seconds defaults to 60; refresh_before_seconds defaults to 300. A literal {session} in a secret URL is replaced with eight random hexadecimal characters on each connection.
